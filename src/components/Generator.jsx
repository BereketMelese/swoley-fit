  ...
  return (
    ...
    >
      <Header
        ...
        description={"Select the workout out you wish to endure."}
      />
      ...
      />
      <div className="grid gird-cols-1 sm:gird-cols-3 gap-4">
        {Object.keys(SCHEMES).map((shceme, shcemeIndex) => {
          return (
            <button
              onClick={() => setGoals(shceme)}
              className={
                "bg-slate-950 border border-blue-400 duration-200 hover:border-blue-600 px-4 py-3 rounded-lg " +
                (shceme === goals ? "border-blue-900" : "border-blue-400")
              }
              key={shcemeIndex}
            >
              <p className="uppercase">{shceme.replaceAll("_", " ")}</p>
            </button>
          );
        })}
      </div>
